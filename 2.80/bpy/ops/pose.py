...
def autoside_names(axis: int = 'XAXIS'):
...
def breakdown(percentage: float = 0.5,
    ...
    :type channels: int
    :param axis_lock: Axis Lock, Transform axis to restrict effects toFREE Free, All axes are affected.X X, Only X-axis transforms are affected.Y Y, Only Y-axis transforms are affected.Z Z, Only Z-axis transforms are affected. 
    :type axis_lock: int
    '''
...
def group_move(direction: int = 'UP'):
...
def propagate(mode: int = 'WHILE_HELD', end_frame: float = 250.0):
...
def select_grouped(extend: bool = False, type: int = 'LAYER'):
