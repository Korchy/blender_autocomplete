...
def anim_transforms_to_deltas():
    ''' Convert object animation for normal transforms to delta transforms :file: startup/bl_operators/object.py\:766 <https://developer.blender.org/diffusion/B/browse/master/release/scripts/startup/bl_operators/object.py$766> _
...
def bake(type: typing.Union[str, int] = 'COMBINED',
    ...
    :param target: Target, Where to output the baked map * IMAGE_TEXTURES Image Textures -- Bake to image data-blocks associated with active image texture nodes in materials. * VERTEX_COLORS Color Attributes -- Bake to active color attribute layer on meshes.
...
def gpencil_modifier_add(type: typing.Union[str, int] = 'GP_THICK'):
    ...
    :param type: Type * GP_TEXTURE Texture Mapping -- Change stroke uv texture values. * GP_TIME Time Offset -- Offset keyframes. * GP_WEIGHT_ANGLE Vertex Weight Angle -- Generate Vertex Weights base on stroke angle. * GP_WEIGHT_PROXIMITY Vertex Weight Proximity -- Generate Vertex Weights base on distance to object. * GP_ARRAY Array -- Create array of duplicate instances. * GP_BUILD Build -- Create duplication of strokes. * GP_DASH Dot Dash -- Generate dot-dash styled strokes. * GP_ENVELOPE Envelope -- Create an envelope shape. * GP_LENGTH Length -- Extend or shrink strokes. * GP_LINEART Line Art -- Generate line art strokes from selected source. * GP_MIRROR Mirror -- Duplicate strokes like a mirror. * GP_MULTIPLY Multiple Strokes -- Produce multiple strokes along one stroke. * GP_SIMPLIFY Simplify -- Simplify stroke reducing number of points. * GP_SUBDIV Subdivide -- Subdivide stroke adding more control points. * GP_ARMATURE Armature -- Deform stroke points using armature object. * GP_HOOK Hook -- Deform stroke points using objects. * GP_LATTICE Lattice -- Deform strokes using lattice. * GP_NOISE Noise -- Add noise to strokes. * GP_OFFSET Offset -- Change stroke location, rotation or scale. * SHRINKWRAP Shrinkwrap -- Project the shape onto another object. * GP_SMOOTH Smooth -- Smooth stroke. * GP_THICK Thickness -- Change stroke thickness. * GP_COLOR Hue/Saturation -- Apply changes to stroke colors. * GP_OPACITY Opacity -- Opacity of the strokes. * GP_TINT Tint -- Tint strokes with new color.
    :type type: typing.Union[str, int]
    '''
...
def hide_render_clear_all():
    ''' Reveal all render objects by setting the hide render flag :file: startup/bl_operators/object.py\:673 <https://developer.blender.org/diffusion/B/browse/master/release/scripts/startup/bl_operators/object.py$673> _
...
def instance_offset_from_cursor():
    ''' Set offset used for collection instances based on cursor position :file: startup/bl_operators/object.py\:855 <https://developer.blender.org/diffusion/B/browse/master/release/scripts/startup/bl_operators/object.py$855> _
...
def isolate_type_render():
    ''' Hide unselected render objects of same type as active by setting the hide render flag :file: startup/bl_operators/object.py\:653 <https://developer.blender.org/diffusion/B/browse/master/release/scripts/startup/bl_operators/object.py$653> _
...
def join_uvs():
    ''' Transfer UV Maps from active to selected objects (needs matching geometry) :file: startup/bl_operators/object.py\:567 <https://developer.blender.org/diffusion/B/browse/master/release/scripts/startup/bl_operators/object.py$567> _
...
def make_dupli_face():
    ''' Convert objects into instanced faces :file: startup/bl_operators/object.py\:641 <https://developer.blender.org/diffusion/B/browse/master/release/scripts/startup/bl_operators/object.py$641> _
...
def modifier_convert(modifier: str = "", merge_customdata: bool = True):
...
def paths_calculate(display_type: typing.Union[str, int] = 'RANGE',
    ...
    :param display_type: Display type * CURRENT_FRAME Around Frame -- Display Paths of poses within a fixed number of frames around the current frame. * RANGE In Range -- Display Paths of poses within specified range.
    :type display_type: typing.Union[str, int]
    :param range: Computation Range * KEYS_ALL All Keys -- From the first keyframe to the last. * KEYS_SELECTED Selected Keys -- From the first selected keyframe to the last. * SCENE Scene Frame Range -- The entire Scene / Preview range.
